...
def transform(df):
    """This function converts the GDP information from Currency format to float
    value, transforms the information of GDP from USD (Millions) to USD (Billions)
    rounding to 2 decimal places. The function returns the transformed dataframe."""

    df["GDP_USD_million"] = df["GDP_USD_million"].str.replace(",", "", regex=False)
    df["GDP_USD_million"] = df["GDP_USD_million"].replace(["—", "-"], "0")
    df["GDP_USD_million"] = df["GDP_USD_million"].astype(float)
